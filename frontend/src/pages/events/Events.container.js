...
const EventsContainer = ({ history }) => {
  ...
  useEffect(() => {
    let availableEventsInCurrentFetch;
    let _showMore;

    if (pastEvents && upcomingEvents) {
      availableEventsInCurrentFetch = totalEventsCount - (pastEvents.length + upcomingEvents.length);
      _showMore = pastEvents.length + upcomingEvents.length !== totalEventsCount;
    } else {
      availableEventsInCurrentFetch = currentFetchEvents.length;
      _showMore = Boolean(currentFetchEvents.length);
    }
